...
/* engineered features from the upstream pipeline, so the model-variable slice below runs */
DATA WORK.Listings_Clean;
   SET WORK.Listings_Clean;
...
DATA WORK.Listings_Clean;
    SET WORK.Listings_Clean;
    IF beds NOT IN (., 0) THEN beds_per_accom = accommodates / beds;
    ELSE beds_per_accom = 0;
    IF bathrooms NOT IN (., 0) THEN bath_per_accom = accommodates / bathrooms;
    ELSE bath_per_accom = 0;
RUN;

/* --- Correlation matrix over the model-variable set (from Airbnb Data Analysis.sas) --- */
...
PROC CORR DATA=WORK.Listings_Clean;
    VAR &model_vars.;
RUN;
